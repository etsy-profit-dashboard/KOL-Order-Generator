...
def resolve_address_row(row, province_items, district_items, geo):
    ...
    # Postal validation / correction:
    # NEVER trust WMS Receiver Zipcode when an authoritative postal code can
    # be derived from the matched subdistrict/district. WMS zipcode is only
    # used as a reference and is accepted only when it belongs to the valid
    # candidate postal codes for that location.
    parts = extract_address_components(address)
    postal = ""
    ...
    if not gprov.empty:
        ...
        # 1) Exact subdistrict is the strongest postal source.
        if sub:
            valid_postals = sorted({
                str(v).strip()
                for v in gprov.loc[gprov["skey"] == sub["key"], "postalCode"]
                if re.fullmatch(r"\d{5}", str(v).strip())
            })
            if len(valid_postals) == 1:
                postal = valid_postals[0]
                postal_status = (
                    "EXACT_SUBDISTRICT"
                    if wms_postal in {"", postal}
                    else "POSTAL_CORRECTED"
                )
            elif len(valid_postals) > 1:
                # If several codes exist, accept WMS/address only when it is
                # one of the valid codes. Never copy an invalid WMS code.
                for candidate in [parts["postal"], wms_postal]:
                    if candidate in valid_postals:
                        postal = candidate
                        postal_status = (
                            "POSTAL_MATCH"
                            if candidate == wms_postal
                            else "ADDRESS_POSTAL_MATCH"
                        )
                        break

        # 2) If subdistrict did not determine the code, try district.
        if not postal:
            gd = gprov[gprov["dkey"] == district["key"]]
            district_postals = sorted({
                str(v).strip()
                for v in gd["postalCode"]
                if re.fullmatch(r"\d{5}", str(v).strip())
            })
            if len(district_postals) == 1:
                postal = district_postals[0]
                postal_status = (
                    "DISTRICT_UNIQUE"
                    if wms_postal in {"", postal}
                    else "POSTAL_CORRECTED"
                )
            elif len(district_postals) > 1:
                # Only use WMS/address when it is a valid code for the
                # matched district. An invalid WMS code is never propagated.
                for candidate in [parts["postal"], wms_postal]:
                    if candidate in district_postals:
                        postal = candidate
                        postal_status = (
                            "POSTAL_MATCH"
                            if candidate == wms_postal
    ...
    # 3) Address postcode is safer than WMS when the address explicitly
    # contains a 5-digit postcode.
    if not postal and parts["postal"]:
        postal = parts["postal"]
        postal_status = (
            "ADDRESS_POSTAL_FALLBACK"
            if parts["postal"] == wms_postal
            else "POSTAL_CORRECTED"
        )

    # 4) WMS zipcode is a last-resort fallback ONLY when no authoritative
    # address/geography source is available. This is visibly marked so the
    # user can review it instead of silently treating bad WMS data as correct.
    if not postal and wms_postal:
        postal = wms_postal
        postal_status = "WMS_POSTAL_FALLBACK_CHECK"

    status = "OK" if postal else postal_status
    if postal_status == "POSTAL_CORRECTED":
        status = "POSTAL_CORRECTED"
    ...
    return {
        ...
        "รหัสไปรษณีย์": postal,
        "รหัสไปรษณีย์จาก WMS": wms_postal,
        "สถานะตรวจสอบ": status,
...
def style_kol_sheet(ws):
    """Final KOL formatting: visible rows, 2-line bilingual headers, correct types."""
    # Never leave rows hidden/outlined.
    ...
    # Remove filter criteria. We will restore only the dropdown range later.
    try:
    ...
    # Header: English on line 1, Chinese on line 2, preserving original styling.
    for col_idx, header in enumerate(KOL_HEADERS, start=1):
        cell = ws.cell(1, col_idx)
        if "/" in header:
            en, zh = header.split("/", 1)
            cell.value = en + "\n" + zh
        else:
            cell.value = header
        old = cell.alignment
        cell.alignment = copy(old)
    ...
    # Phone fields are text so leading zero is retained; exactly 10 digits.
    for cell in ws["P"][2:]:
        cell.number_format = "@"
        cell.value = clean_mobile_number(cell.value)
    for cell in ws["Q"][2:]:
        cell.number_format = "@"
        cell.value = clean_mobile_number(cell.value)

    # Postal code must be a real Excel number, not text.
    for cell in ws["V"][2:]:
        postal = valid_postcode(cell.value)
        cell.value = int(postal) if postal else ""
        cell.number_format = "0"

    # Match the known-good template date display: m/d/yyyy;@.
    for col in ("M", "N"):
        for cell in ws[col][2:]:
            cell.number_format = "m/d/yyyy;@"


def force_wps_open_at_a1(buffer):
    """Patch the XLSX XML directly so WPS cannot reopen at the old row 478 view."""
    import zipfile
    import xml.etree.ElementTree as ET

    src = io.BytesIO(buffer.getvalue())
    out = io.BytesIO()
    with zipfile.ZipFile(src, "r") as zin, zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zout:
        for item in zin.infolist():
            ...
            if item.filename == "xl/worksheets/sheet1.xml":
                ...
                # The embedded WPS template contains topLeftCell=I478 and activeCell=Q484.
                text = re.sub(r'topLeftCell="[^"]*"', 'topLeftCell="A1"', text)
                text = re.sub(r'<selection[^>]*/>', '<selection activeCell="A1" sqref="A1"/>', text)
                # If selection is absent, insert one inside sheetView.
                if '<selection' not in text:
                    text = text.replace('</sheetView>', '<selection activeCell="A1" sqref="A1"/></sheetView>', 1)
                data = text.encode("utf-8")
            zout.writestr(item, data)
    buffer.seek(0)
    buffer.truncate(0)
    buffer.write(out.getvalue())
    buffer.seek(0)
...
if not missing_weight_df.empty:
    with st.expander(f"⚠️ SKU ไม่มีข้อมูลน้ำหนัก ({len(missing_weight_df):,} รายการ)", expanded=False):
        st.dataframe(missing_weight_df, width="stretch", hide_index=True)

# Preview
st.subheader("ตัวอย่าง KOL ที่จะดาวน์โหลด")
st.dataframe(kol_df.head(100), width="stretch", hide_index=True)
...
# FINAL WPS/Excel VIEW CLEANUP
# Ensure no sheet retains hidden rows, split panes, old scroll positions,
# or an old active cell from the embedded template.
for _ws in kol_wb.worksheets:
    _ws.freeze_panes = None
    try:
        _ws.auto_filter.filterColumn = []
    except Exception:
        pass
    for _r in range(1, _ws.max_row + 1):
        _ws.row_dimensions[_r].hidden = False
        _ws.row_dimensions[_r].outlineLevel = 0
    _sv = _ws.sheet_view
    _sv.topLeftCell = "A1"
    if _sv.selection:
        _sel = _sv.selection[0]
        _sel.activeCell = "A1"
        _sel.sqref = "A1"
        _sel.pane = None

# Make the KOL sheet the active sheet and force workbook-level active view.
kol_wb.active = 0
try:
    kol_wb.views[0].activeTab = 0
except Exception:
    pass

kol_wb.save(buffer)

# Final XML-level cleanup: force WPS/Excel to reopen at A1.
force_wps_open_at_a1(buffer)

st.success("สร้างไฟล์ KOL สำเร็จ — เปิดที่แถว 1 พร้อมรูปแบบที่แก้ไขแล้ว")
st.download_button(
    "⬇️ ดาวน์โหลด KOL_Upload_Ready_FINAL.xlsx",
    data=buffer.getvalue(),
    file_name="KOL_Upload_Ready_FINAL.xlsx",
