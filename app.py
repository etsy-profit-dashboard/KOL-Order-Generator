...
def _fix_new_kol_weight_formula(wb, data_rows):
    """
    Fix the KOL NEW weight helper formula (column AE) for the real data rows.

    The original formula multiplies the weight cell by Allocated Qty directly.
    If Allocated Qty is stored as Excel text (for example "1") and the SKU
    weight is exactly 0, some spreadsheet engines can return #VALUE!.

    Use VALUE() + IFERROR() so zero-weight SKUs remain numeric 0 and the
    package-weight formula in AA can continue calculating normally.
    """
    sheet_name = "ไฟล์อัพโหลด KOL ระบบเก่า"
    if sheet_name not in wb.sheetnames:
        return

    ws = wb[sheet_name]
    first_row = 3
    last_row = first_row + max(0, int(data_rows)) - 1
    if last_row < first_row:
        return

    for r in range(first_row, last_row + 1):
        # The OLD KOL formula row r maps to NEW KOL order row r-1.
        weight_row = r - 1
        ws[f"AE{r}"] = (
            f'=IF(ข้อมูลน้ำหนักสินค้า!C{weight_row}="","",'
            f'IFERROR(VALUE(ข้อมูลน้ำหนักสินค้า!C{weight_row})*VALUE(J{r}),0))'
        )
...
try:
    ...
    with st.spinner("กำลังสร้างไฟล์ KOL ระบบใหม่และ KOL ระบบเก่า..."):
        ...
        # Fix the one NEW KOL weight-formula issue:
        # text quantity + zero SKU weight must not produce #VALUE!.
        _fix_new_kol_weight_formula(new_kol_wb, len(final))
    ...
    with col2:
        ...
        st.caption(
            "แยกเฉพาะ 30 คอลัมน์ A:AD ของชีท 'ไฟล์อัพโหลด KOL ระบบเก่า' "
            "แปลงสูตรเป็นค่าข้อมูลจริง ไม่มีสูตร Excel และไม่ส่งออกคอลัมน์ AE"
        )
